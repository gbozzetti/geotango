...
function et_home_posts_query( $query = false ) {
	...
	$query->set( 'post__not_in', date_eventi());
}
...
function et_explorable_taxonomy_query( $query = false ) {
	...
	if ( is_post_type_archive( 'listing' ) ) {
		...
		if ( isset( $_GET['et-listing-date'] )  ) {
			$query->set( 'post__not_in', date_eventi($_GET['et-listing-date']) );
		}
	}
}
...
function date_eventi($value) {
	$array_dates = listing_dates();
	if ($value) {
		$key = strtolower(array_search($value, $array_dates));
	}
	echo $key;
	$current_time = strtotime(date_i18n('Ymd', current_time( 'timestamp' )));
	$now = strtotime(date_i18n('Ymd', current_time( 'timestamp' )));
	$the_query = new WP_Query( 'post_type=listing' );
	$id_posts = array();
	if ($value) { 
		if ($value == 'none') {
			$current_time = $now;
	...
	while ( $the_query->have_posts() ) :
		$the_query->the_post();
		$value = get_field( "data_evento", get_the_ID() );
		$timestamp = strtotime($value);
		if ($value) {
			$fine_evento = get_field( "fine_evento", get_the_ID() );
			if ($fine_evento) { $timestamp = strtotime($fine_evento); }
			if ($key) {
				if ($key == 'today') {
					$start_event = strtotime($value);
					$end_event = $timestamp;
					$start_time = $now;
					$end_time = $now;
					if ($end_event < $start_time || $now < $start_event) {
						array_push($id_posts, get_the_ID());
					}
				} elseif ($key == 'tomorrow') {
					$start_event = strtotime($value);
					$end_event = $timestamp;
					$start_time = $array_dates['Tomorrow'];
					$end_time = $array_dates['Tomorrow'];
					if ($end_event < $start_time || $now < $start_event) {
						array_push($id_posts, get_the_ID());
					}
				} elseif ($key == 'this week') {
					$start_event = strtotime($value);
					$end_event = $timestamp;
					$start_time = strtotime("Monday this week");
					$end_time = $array_dates['This Week'];
					if ($end_event < $start_time || ($now < $start_event && $start_event > $end_time) || ($now > $start_event && $end_event < $now) ) {
						array_push($id_posts, get_the_ID());
					}
					echo $now.'<br />';
				} elseif ($key == 'next week') {
					$start_event = strtotime($value);
					$end_event = $timestamp;
					$start_time = strtotime("+7 days", strtotime("Monday this week"));
					$end_time = $array_dates['Next Week'];
					if ($end_event < $start_time || ($now < $start_event && $start_event > $end_time) ) {
						array_push($id_posts, get_the_ID());
					}
				}
...
function listing_dates() {
	$current_time = strtotime(date_i18n('Ymd', current_time( 'timestamp' )));
	$tomorrow = strtotime("+1 day", $current_time);
	$this_week_end = strtotime("sunday", $current_time);
	$next_week_end = strtotime("+7 days", $this_week_end);
	$array_dates['Today'] = $current_time;
	$array_dates['Tomorrow'] = $tomorrow;
	if ($current_time != $this_week_end) {
	$array_dates['This Week'] = $this_week_end;
	}
	$array_dates['Next Week'] = $next_week_end;
	return $array_dates;
}
