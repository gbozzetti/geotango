(function($){
	...
	$(document).ready( function(){
		...
		if ( $et_filter_form.length ) {
			...
			$et_filter_listing_date = $et_filter_form.find( '#et-listing-date' );

			$et_filter_listing_date.find( 'option' ).each( function() {
				var $this_option = $( this );

				et_filter_options_html += '<li data-value="' + $this_option.attr( 'value' ) + '">' + $this_option.text() + '</li>';
			} );

			$( 'a.listing-date' ).append( '<ul>' + et_filter_options_html + '</ul>' );
			...
			if ( $et_filter_listing_date.find( ':selected' ).length ) {
				$( 'a.listing-date .et_explorable_filter_text' ).text( $et_filter_listing_date.find( ':selected' ).text() );
			}
