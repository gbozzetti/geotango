...
?>
		...
		function et_add_marker( marker_order, marker_lat, marker_lng, marker_description, marker_type ){
			var marker_id = 'et_marker_' + marker_order;
			if (marker_type == 'Milonga') {
				m_type = 'green-marker.png';
			} else if (marker_type == 'Festival') {
				m_type = 'red2-marker.png';
			} else {
				m_type = 'red-marker.png';
			}
			...
			$et_main_map.gmap3({
				marker : {
					...
					options: {
						icon : "<?php echo get_template_directory_uri(); ?>/images/" + m_type,
						type : marker_type
					},
					events : {
						click: function( marker ){
							if ( et_active_marker ){
								if (et_active_marker.type == 'Milonga') {
									m_type = 'green-marker.png';
								} else if (et_active_marker.type == 'Festival') {
									m_type = 'red2-marker.png';
								} else {
									m_type = 'red-marker.png';
								}
								et_active_marker.setAnimation( null );
								et_active_marker.setIcon( '<?php echo get_template_directory_uri(); ?>/images/' + m_type);
							}
...
while ( have_posts() ) : the_post();
	...
	if ( '' != $et_location_lat && '' != $et_location_lng ) {
?>
			et_add_marker( <?php printf( '%1$d, %2$s, %3$s, \'<div id="et_marker_%1$d" class="et_marker_info"><div class="location-description"> <div class="location-title"> <h2>%4$s</h2> <div class="listing-info"><p>%5$s</p></div> </div> ' . $et_location_rating . ' </div> <!-- .location-description --> </div> <!-- .et_marker_info -->\', \'%5$s\'',
				$i,
...
?>
		...
		<?php if ( ''!= $thumb ) { ?>
			...
			<?php
				$term_list = wp_get_post_terms($post->ID, 'listing_type', array("fields" => "slugs"));
				if ($term_list[0] == 'milonga') { $term = 'milonga'; }
				if ($term_list[0] == 'festival') { $term = 'festival'; }
				$fine_evento = get_field('fine_evento');
				if (has_term('scuola', 'listing_type')) {
					printf( '<div class="et-date-wrapper"><span class="et-date scuola">Scuola</span></div>');
				}
				elseif ($fine_evento) {
					printf( '<div class="et-date-wrapper"><span class="et-date %s"><span>dal </span>%s<span>%s</span><br /><span>al </span>%s<span>%s</span></span></div>',
					$term,
					date_i18n('j F', strtotime(get_field('data_evento'))),
					date_i18n('Y', strtotime(get_field('data_evento'))),
					date_i18n('j F', strtotime(get_field('fine_evento'))),
					date_i18n('Y', strtotime(get_field('fine_evento')))
					);
				} else {
				printf( '<div class="et-date-wrapper"><span class="et-date %s">%s<span>%s</span></span></div>',
					$term,
					date_i18n('F j', strtotime(get_field('data_evento'))),
					date_i18n('Y', strtotime(get_field('data_evento')))
...
<div id="filter-bar">
	<div class="container">
		<form method="get" id="et-filter-map" action="<?php echo esc_url( home_url( '/' ) ); ?>">
			...
			<a href="#" class="filter-type listing-date"><span class="et_explorable_filter_text"><?php esc_html_e( 'Any Date', 'Explorable' ); ?></span><span class="et_filter_arrow"></span></a>
...
?>
			...
			<select name="et-listing-date" id="et-listing-date">
				<option value="none"><?php esc_html_e( 'Any Date', 'Explorable' ); ?></option>
<?php
		if ( $listing_dates ) {
			foreach( $listing_dates as $key => $listing_date ) {
				printf( '<option value="%d"%s>%s</option>',
					esc_attr( $listing_date ),
					( isset( $_GET['et-listing-date'] ) && 'none' != $_GET['et-listing-date'] ? selected( intval( $_GET['et-listing-date'] ), $listing_date, false ) : '' ),
					esc_html( $key )
				);
			}
		}
